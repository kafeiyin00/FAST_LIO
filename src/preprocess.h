...
// 枚举类型：表示支持的雷达类型
enum LID_TYPE{AVIA = 1, VELO16, OUST64, MARSIM}; //{1, 2, 3, 4}
...
//   EIGEN_MAKE_ALIGNED_OPERATOR_NEW
  ...
  PointCloudXYZI pl_buff[128];               // 最多128线雷达
  vector<orgtype> typess[128];               // 最多128线雷达的特征类型
  float time_unit_scale;                     // 时间单位缩放因子
  // 雷达类型、采样间隔、扫描线数、扫描频率、时间单位
  ...
  // 特征提取相关参数
  int group_size;                    // 分组大小
  double disA, disB, inf_bound;      // 距离参数和无穷大边界
  double limit_maxmid, limit_midmin, limit_maxmin; // 阈值限制参数
  double p2l_ratio;                  // 点到线的比例
  double jump_up_limit, jump_down_limit;  // 跳变上下限
  double cos160;                     // cos(160°)值
  double edgea, edgeb;               // 边缘参数a和b
  double smallp_intersect, smallp_ratio; // 小平面相交角和比例
