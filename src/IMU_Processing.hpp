...
class ImuProcess
{
 public:
  ...
  V3D cov_acc_scale;       //加速度测量协方差缩放因子
  V3D cov_gyr_scale;       //角速度测量协方差缩放因子
  V3D cov_bias_gyr;        //角速度偏置协方差
  V3D cov_bias_acc;        //加速度偏置协方差
  double first_lidar_time; //第一个点云时间戳
  double IMU_mean_acc_norm = 9.81; //IMU加速度均值的模
 ...
 private:
  ...
  PointCloudXYZI::Ptr cur_pcl_un_;   //当前帧点云未成伧的查影
  sensor_msgs::ImuConstPtr last_imu_;  //上一帧IMU最后的一个值
  deque<sensor_msgs::ImuConstPtr> v_imu_; // IMU数据队列
  vector<Pose6D> IMUpose;              //IMU位姿数据
  vector<M3D>    v_rot_pcl_;           //未使用
  M3D Lidar_R_wrt_IMU;                 // Lidar相寸IMU的旋转外参
  V3D Lidar_T_wrt_IMU;                 // Lidar相寸IMU的位置外参
  V3D mean_acc;                        //加速度测量均值，用于计算方差
  V3D mean_gyr;                        //角速度测量均值，用于计算方差
  V3D angvel_last;                     //上一帧角速度
  V3D acc_s_last;                      //上一帧ℯ加速度
    ...
    : b_first_frame_(true), imu_need_init_(true), start_timestamp_(-1)
{
  ...
  Q = process_noise_cov();              //调用use-ikfom.hpp中的process_noise_cov初始化嘿声协方差
  cov_acc       = V3D(0.1, 0.1, 0.1);   //加速度测量协方差初始化
  cov_gyr       = V3D(0.1, 0.1, 0.1);   //角速度测量协方差初始化
  cov_bias_gyr  = V3D(0.0001, 0.0001, 0.0001); //角速度偏置协方差初始化
  cov_bias_acc  = V3D(0.0001, 0.0001, 0.0001); //加速度偏置协方差初始化
  mean_acc      = V3D(0, 0, -1.0);      //加速度均值初始化（重力方向）
  mean_gyr      = V3D(0, 0, 0);         //角速度均值初始化
  angvel_last     = Zero3d;             //上一帧角速度初始化
  Lidar_T_wrt_IMU = Zero3d;             // Lidar相寸IMU的位置外参初始化
  Lidar_R_wrt_IMU = Eye3d;              // Lidar相寸IMU的旋转外参初始化
...
void ImuProcess::Reset() 
{
  ...
  mean_acc      = V3D(0, 0, -1.0);  //加速度均值复位
  mean_gyr      = V3D(0, 0, 0);     //角速度均值复位
  angvel_last       = Zero3d;       //上一帧角速度复位
  imu_need_init_    = true;         //是否需要初始化imu
  start_timestamp_  = -1;           //开始时间戳复位
  init_iter_num     = 1;            //初始化迭代次数复位
...
void ImuProcess::IMU_init(const MeasureGroup &meas, esekfom::esekf<state_ikfom, 12, input_ikfom> &kf_state, int &N)
{
  ...
  if (b_first_frame_)
  {
    ...
    //从common_lib.h中拿到imu初始时刻的加速度
    const auto &imu_acc = meas.imu.front()->linear_acceleration;
    //从common_lib.h中拿到imu初始时刻的角速度
  ...
  //将协方差矩阵置为单位阵
  init_P.setIdentity();
  //将协方差矩阵的位置和旋转的协方差置为0.00001
  init_P(6,6) = init_P(7,7) = init_P(8,8) = 0.00001;
  //将协方差矩阵的速度和位姿的协方差置为0.00001
  init_P(9,9) = init_P(10,10) = init_P(11,11) = 0.00001;
  //将协方差矩阵的重力和姿态的协方差置为0.0001
  init_P(15,15) = init_P(16,16) = init_P(17,17) = 0.0001;
  //将协方差矩阵的陀螺仪偏差和姿态的协方差置为0.001
  init_P(18,18) = init_P(19,19) = init_P(20,20) = 0.001;
  //将协方差矩阵的lidar和imu外参位移量的协方差置为0.00001
  init_P(21,21) = init_P(22,22) = 0.00001; 
  //将初始化协方差矩阵传入esekfom.hpp中的P_
  kf_state.change_P(init_P);
  //将最后一帧的imu数据传入last_imu_中，在UndistortPcl使用到了
...
void ImuProcess::UndistortPcl(const MeasureGroup &meas, esekfom::esekf<state_ikfom, 12, input_ikfom> &kf_state, PointCloudXYZI &pcl_out)
{
  ...
  /*** 在进行完所有的IMU预测后，剩下的就是对激光的去畸变了 ***/
  /*** 基于IMU预测对lidar点云去畸变（反向传播） ***/
...
// ==================== IMU处理主函数 ====================
// 功能：处理一帧LiDAR点云对应的IMU数据，完成初始化或去畸变
// 
// 工作流程：
//   1. 如果IMU未初始化（imu_need_init_==true）
//      ├─ 调用IMU_init()：利用静止状态下的IMU数据初始化
//      └─ 构建EKF的初始状态和协方差矩阵
//   2. 如果IMU已初始化（imu_need_init_==false）
//      ├─ 调用UndistortPcl()：进行IMU前向传播和点云反向去畸变
//      └─ 得到在同一时刻（通常是扫描末尾）去畸变后的点云
//
// 参数说明：
//   meas: 当前帧的测量数据（包含LiDAR点云和对应IMU序列）
//   kf_state: ESEKF滤波器状态（维度：23状态 + 12噪声）
//   cur_pcl_un_: 输出的去畸变后点云（原始点云的去畸变版本）
//
// 关键变量：
//   imu_need_init_: 标志位，表示IMU是否需要初始化
//   init_iter_num: 初始化迭代计数器
//   last_imu_: 上一帧最后一个IMU数据
//   last_lidar_end_time_: 上一帧LiDAR扫描的结束时间
void ImuProcess::Process(const MeasureGroup &meas,  esekfom::esekf<state_ikfom, 12, input_ikfom> &kf_state, PointCloudXYZI::Ptr cur_pcl_un_)
{
  ...
  if (imu_need_init_)
  {
    /// 第一个激光雷达帧 初始化重力、陀螺偏差、加速度和陀螺仪协方差
